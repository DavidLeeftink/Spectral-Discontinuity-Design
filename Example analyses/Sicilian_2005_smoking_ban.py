...
cosine_linear           = GPy.kern.Cosine(D) + GPy.kern.Linear(D)
#_                       = std_periodic_kernel['period'].constrain_fixed()
matern32_kernel         = GPy.kern.Matern32(D)
RBF_kernel              = GPy.kern.RBF(D)
periodic                = GPy.kern.PeriodicExponential(D)
gabor                   = GPy.kern.PeriodicMatern52(D) * GPy.kern.RBF(D)
rat_quad                = GPy.kern.RatQuad(D)
...
kernel_names    = ['Rational Quadratic', 'Cosine linear','Periodic', 'Gabor', 'RBF']
kernels         = [rat_quad, cosine_linear, periodic, gabor, RBF_kernel]

kernel_dict = dict(zip(kernel_names, kernels))

opts = dict()
opts['num_restarts'] = 10 # 50 is the old value, but it barely changes after 2..
opts['mode'] = 'BIC'
opts['verbose'] = False
