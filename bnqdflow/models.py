...
#             print(f'BIC: {BIC}')
    ...
    def train(self, optimizer: Any = optimizers.Scipy(), verbose=True) -> None:
        ...
        optimizer.minimize(self._training_loss, self.trainable_variables, method = 'BFGS')
    ...
    def plot_regression(self, n_samples=100, padding: Union[float, Tuple[float]] = 0.0, num_f_samples=5, plot_data=True,
                        predict_y=False, ax=None, ylim=None, lineplot=False, X_transform = None, Y_transform=None):
        """
        ...
        colours = ['#9b59b6', '#3498db', 'red', 'cyan', 'magenta', 'yellow', 'orange', 'purple']
        ...
        for ip in self.intervention_points:
            ax.axvline(ip, linestyle='--', c='k')

        if plot_data:
            # Plots the training data with different markers per section.
            for i, ((x, y), m) in enumerate(zip(self.data_list, cycle(markers))):
                # Rescale X and Y back before normalization  
                if lineplot:
                    ax.plot(x[:, 0], y[:, 0], color='black', linewidth=2.0,label=f'$obs_{i}$')
                else:
                    ax.plot(x[:, 0], y[:, 0], linestyle='none', marker=m, color='k', label=f'$obs_{i}$')
...
class ContinuousModel(GPMContainer):
    ...
    def plot_regression(self, n_samples=100, padding: Union[float, Tuple[float]] = 0.0, num_f_samples=5, plot_data=True,
                        predict_y=False, ax=None, ylim=None, lineplot=False):
        """
        ...
        if ylim is not None:
            ax.set_ylim(ylim[0], ylim[1])
        # continuousmodelcolor
        colours = ['#34495e','#05712f', '#3498db', 'red', 'cyan', 'magenta', 'yellow', 'orange', 'purple']
        col = colours[(self.n_models - 1) % len(colours)]
        markers = ['x', '+', '.', '*', 'd', 'v', 's', 'p', 'X', 'P', 'h']
        ...
        if plot_data:
            x, y = self.data
            if lineplot:
                ax.plot(x[:, 0], y[:, 0], color='black', label=f'$obs$')
            else:
                ax.plot(x[:, 0], y[:, 0], linestyle='none', marker=markers[0], color='k', label=f'$obs$')
        ...
        if num_f_samples > 0 and not predict_y:
            ...
            for f_sample in f_samples:
                ax.plot(x_samples, f_sample[:, 0], linewidth=0.2, c=col)
